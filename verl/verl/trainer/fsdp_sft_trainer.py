...
class FSDPSFTTrainer(object):
    ...
    def _build_dataloader(self):
        config = self.config
        # build dataset
        task = getattr(config.data, 'task', 'sft')
        assert task in ['sft', 'agent'], 'config.data.task must be choice of [sft|agent]'
        dataset_func = SFTDataset
        self.train_dataset = dataset_func(parquet_files=config.data.train_files,
                                        tokenizer=self.tokenizer,
                                        system_key=config.data.system_key,
                                        system_dict_keys=config.data.get('system_dict_keys', None),
        ...
        self.val_dataset = dataset_func(parquet_files=config.data.val_files,
                                      tokenizer=self.tokenizer,
                                      system_key=config.data.system_key,
                                      system_dict_keys=config.data.get('system_dict_keys', None),
                                      prompt_key=config.data.prompt_key,
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, verbose=False):
        ...
        with context:
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                if not use_sp:
                    ...
                    if verbose:
                        output_ids = torch.argmax(logits_rmpad,dim=-1)
                        # print(logits_rmpad.shape)
                        print(self.tokenizer.batch_decode([output_ids], skip_special_tokens=False))
                    
                    update = torch.zeros_like(logits_rmpad)
                    mask = input_ids_rmpad_rolled == 151665  # 找到满足条件的索引
                    update[mask, 151665] = update[mask, 151665]*0 - 5
                    logits_rmpad = logits_rmpad + update
                    del update
                    # label = input_ids_rmpad_rolled
                    # label = gather_outpus_and_unpad(label, gather_dim=0, unpad_dim=0, padding_size=pad_size)
                    # label = pad_input(hidden_states=label.unsqueeze(-1),
                    #                       indices=indices,
                    #                       batch=batch_size,
                    #                       seqlen=seqlen)
                    ...
                    full_loss = pad_input(hidden_states=loss.unsqueeze(-1),
                                          ...
                                          batch=batch_size,
                                          seqlen=seqlen)
                    # print(label.squeeze(-1)[:, :-1].reshape(-1)[loss_mask==100], full_loss.squeeze(-1)[:, :-1].reshape(-1)[loss_mask==100])
    ...
    def training_step(self, batch: TensorDict):
        ...
        step_loss = 0
        for ind,micro_batch in enumerate(micro_batches):
            loss = self._compute_loss_and_backward(batch=micro_batch, verbose=ind==n_micro_batches-1) / n_micro_batches
            step_loss += loss.item()
